...
KDNodePtr NewKDNodePtr() {
    KDNodePtr mynode = make_shared< KDNode >();
    return mynode;
}
...
inline double dist2(const point_t &a, const point_t &b) {
    double distc = 0;
    auto entryB = b.begin();
    for (auto const &entryA: a) {
        double di = entryA - (*entryB);
        distc += di * di;
        entryB++;
    }
    return distc;
...
inline pair < double, point_t > dist2_and_diff(
        const point_t &a, 
        const point_t &b)
{
    double _dist2 = 0;
...
inline void sort_on_idx(const pointIndexArr::iterator &begin,  //
    ...
    nth_element(begin, begin + distance(begin, end) / 2,
                     end, bind(&comparer::compare_idx, comp, _1, _2));
}
...
KDTree::KDTree(pointVec point_array) {
    leaf = make_shared< KDNode >();
    // iterators
    pointIndexArr arr;
    auto point_array_it = point_array.begin();
    for (size_t i = 0; i < point_array.size(); i++) {
        arr.push_back(pointIndex(*point_array_it, i));
        ++point_array_it;
    }
...
vector <tuple< vector< double >, double, size_t > >  KDTree::neighborhood_(  //
    const KDNodePtr &branch,          //
    const point_t &pt,                //
    const double &rad,                //
    const size_t &level               //
    ...
    if (!bool(*branch)) {
        ...
        // no points to add
        return  vector <tuple< vector< double >, double, size_t > > ();
    }

    size_t dim = pt.size();

    double r2 = rad * rad;
    pair < double, vector < double > > d2_and_diff =  dist2_and_diff(point_t(*branch), pt);
    dx = d2_and_diff.second.at(level);
    dx2 = dx * dx;

    if (d2_and_diff.first == 0.0){
        // query point is equal to match, a case we want to ignore
        return  vector <tuple< vector< double >, double, size_t > > ();
    }

    vector <tuple< vector< double >, double, size_t > >  nbh, nbh_s, nbh_o;
    if (d2_and_diff.first <= r2) {
        nbh.push_back(make_tuple(
                            d2_and_diff.second,
                            d2_and_diff.first,
                            branch->index
                      ));
...
vector <tuple< vector< double >, double, size_t > >  KDTree::neighborhood(  //
    const point_t &pt,               //
    const double &rad) {
    size_t level = 0;
